...
SCRP_ERROR = br"\$[\w\W]*\$(\d{1,})\s{1}([a-zA-Z\s]*)\n!"
...
# NOTE: Add 'fullmatch=True' to NET response generators. Otherwise, response will contain
#       prepended data.
...
@hs.composite
def dict_key_and_value(draw, devices=hs.from_regex(FGC_NAME), dev_resps=hs.from_regex(NET_SUCCESS, fullmatch=True), max_size=None, errors_only=False):
    if errors_only:
        dev_resps = hs.from_regex(NET_ERROR, fullmatch=True)
